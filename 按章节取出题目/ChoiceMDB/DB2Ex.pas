...
type
    TForm1 = class(TForm)
        ...
        btn12: TButton;
        chk11: TCheckBox;
        chk_disp_answer: TCheckBox;
        chk_disp_anay: TCheckBox;
    img1: TImage;
    chk_sgn_answer: TCheckBox;
    edtAnswer1: TRxRichEdit;
...
var
    ...
    is3ji: Boolean = False;
    is2c: boolean = False;
    TableName: array[0..3] of string = ('二级C语言一级考点表',
        '二级C语言二级考点表', '二级C语言三级考点表', '二级C语言四级考点表');
...
begin
    ...
    ti.SQL.Clear;
    ti.SQL.Add('select TH,FZ,考点1,考点1比例,考点2,考点2比例,考点3,考点3比例,考点4,考点4比例,考点5,考点5比例,考点6,考点6比例 from ' + table);
    ti.open;
    cou := ti.RecordCount;
    for i := 1 to cou do
    begin
...
begin
    ...
    begin
        MessageBox(0, PCHar('数据表:''' + TableName[1] + '''似乎不存在!' + #13 +
            #13
            + '请检查数据库!'),
    ...
    Setlength(s, MAX + 1);
    if Form1.rg1.itemindex = 0 then
        table := '题目表'
    else
        table := '模拟题表';
    if not (TableExist(DBe.DM.con1, Table)) then
    begin
        MessageBox(0, PCHar('数据表:''' + Table + '''似乎不存在!' + #13 + #13 +
            '请检查数据库!'),
            '出错了', MB_OK or MB_ICONWARNING);
        exit;
    end;
    ...
    aq3.SQL.Clear;
    aq4.SQL.Clear;
    AQ1.SQL.Add('select ' + field1 + ' from ' + TableName[0] +
        '  order by 一级考点ID ASC');
    AQ2.SQL.Add('select ' + field2 + ' from ' + TableName[1] +
        '  order by 二级考点ID ASC');
    AQ3.SQL.Add('select ' + field3 + ' from ' + TableName[2] +
        '  order by 三级考点ID ASC');
    AQ4.SQL.Add('select ' + field4 + ' from ' + TableName[3] +
        '  order by 四级考点ID ASC');
    AQ1.open;
    aq2.Open;
    aq3.Open;
    ...
    for i := 1 to ticou do
    begin
        if (ti.FieldByName('FZ').asInteger <= 0) or
            (ti.FieldByName('FZ').asInteger
            > 10) or (ti.FieldByName('考点1比例').AsFloat <= 0) or
            (ti.FieldByName('考点1比例').AsFloat > 1) then
        begin
            if crow = 1 then
            begin
                try
...
begin
    ...
    begin
        MessageBox(0, PCHar('数据表:''' + PublicTable[1] + '''似乎不存在!' + #13
            +
            #13 + '请检查数据库!'),
            '出错了', MB_OK or MB_ICONWARNING);
        exit;
    ...
    aq3.SQL.Clear;
    aq4.SQL.Clear;
    AQ1.SQL.Add('select ' + field1 + ' from ' + PublicTable[0] +
        '  order by 一级考点ID ASC');
    AQ2.SQL.Add('select ' + field2 + ' from ' + PublicTable[1] +
        '  order by 二级考点ID ASC');
    AQ3.SQL.Add('select ' + field3 + ' from ' + PublicTable[2] +
        '  order by 三级考点ID ASC');
    AQ4.SQL.Add('select ' + field4 + ' from ' + PublicTable[3] +
        '  order by 四级考点ID ASC');
    AQ1.open;
    aq2.Open;
    aq3.Open;
...
begin
    ...
    TableXiaBiao := 'a';
    i := ord(TableXiaBiao) + MAX + 2; //确定列的范围
    TableXiaBiao := chr(i);
    try
        v := CreateOleObject('Excel.Application');
        //如果这个字符串中有空格就会出'无效的类别字符串'的错
        ...
        if is3ji then
        begin
            v.range['b2:' + TableXiaBiao + '2'].FormatConditions.Add(1, 1, '0',
                '99.995');
            v.range['b2:' + TableXiaBiao +
                '2'].FormatConditions[1].Font.ColorIndex :=
                3;
            v.range['b2:' + TableXiaBiao + '2'].FormatConditions.Add(1, 7,
                '100.005');
        end
        else
        begin
        ...
        v.range['b2:' + TableXiaBiao + '2'].FormatConditions[2].Font.ColorIndex
            :=
            3;
    ...
    begin
        ...
        begin
            ...
            row1 := crow;
            v.range['a' + inttostr(crow) + ':' + TableXiaBiao +
                inttostr(crow)].interior.colorindex := 24;
            str := aq1.fieldbyname('一级考点名称').asstring;
            Sheet.cells[crow, 1] := ' ' +
                aq1.fieldbyname('一级考点名称').asstring;
            ...
            if cou2 > 0 then
                ...
                begin
                    ...
                    row2 := crow;
                    Sheet.cells[crow, 1] := '     ' +
                        aq2.fieldbyname('二级考点名称').asstring;
                    str := aq2.fieldbyname('二级考点名称').asstring;
                    str := copy(str, Pos(' ', str) + 1, Length(str) - pos(' ',
                        str));
                    getsum(2, str, Table);
                    for k := 1 to MAX do
                        Sheet.cells[crow, k + 1] := sum[2, k];
                    aq3.sql.clear;
                    aq3.sql.add('select ' + field3 + ' from ' + TableName[2] +
                        ' where ([三级考点所属一级考点ID]=' + inttostr(i) +
                        ') and ([三级考点所属二级考点ID]=' + inttostr(j) +
                        ') order by 三级考点ID ASC');
                    aq3.open;
                    cou3 := aq3.RecordCount;
                    for m := 1 to MAX do
                    ...
                    if cou3 > 0 then
                        ...
                        begin
                            ...
                            row3 := crow;
                            Sheet.cells[crow, 1] := '         ' +
                                aq3.fieldbyname('三级考点名称').asstring;
                            ...
                            aq4.sql.clear;
                            aq4.sql.add('select ' + field4 + ' from ' +
                                TableName[3] +
                                ' where ([四级考点所属一级考点ID]=' + inttostr(i)
                                    +
                            ...
                            if cou4 > 0 then
                                ...
                                begin
                                    ...
                                    crow := crow + 1;
                                    Sheet.cells[crow, 1] := '             ' +
                                        aq4.fieldbyname('四级考点名称').asstring;
                                    str :=
                                        aq4.fieldbyname('四级考点名称').asstring;
                    ...
                    begin
                        ...
                        if sum[2, l] > 0 then
                            counter := counter + 1; //大于0就计数，用来计算概率
                    end;
            ...
            begin //计算合计
                ...
                if sum[1, l] > 0 then
                    counter := counter + 1; //大于0的值就计数，准备计算概率
            end;
    ...
    begin
        ...
        if not is3ji then
            //         if is2c then
            sqlstr := sqlstr + ' and ((XH>10 And XH<41) )';
        //         else           //如果当前不是C，也不是3级，那就是二级其他科目
        //           sqlstr:=sqlstr+' and ((XH>10 And XH<36) OR (XH>40))';
        Application.Restore;
        Application.BringToFront;
        if MessageBox(0, '发现不和谐的数据，要把它拿出来晒一晒吗', '注意',
...
begin
    ...
    begin
        ...
        MessageBox(0, '没有找到题目表，请检查数据库', '没题目表', MB_OK);
    for i := 0 to 4 do
    begin
        if TableExist(DBe.DM.con1, table[i]) then
            Break;
    end;
    case i of
        0:
        ...
        4:
            begin
                ...
                TableName[3] := '三级网络技术四级考点表';
                s2 := '三级网络技术';
                is3ji := True;
            end;
    else
        MessageBox(0, '数据库错误,没有找到可用的表,本程序将退出.', '检查数据库',
            MB_OK);
        Halt;
    end;
...
begin
    ...
    try
        ...
        begin
            ...
            sht[i].range['b:' + TableXiaBiao].FormatConditions[1].Font.ColorIndex
                :=
                15;
        end;
    ...
    begin
        ...
        begin
            ...
            v.range['a' + inttostr(row1) + ':' + TableXiaBiao +
                inttostr(row1)].interior.colorindex := 24;
            str := aq1.fieldbyname('一级考点名称').asstring;
            //sht[1].cells[row1,1]:=aq1.fieldbyname('一级考点名称').asstring;
            str := copy(str, Pos(' ', str) + 1, Length(str) - pos(' ', str));
            getsum(1, str, Table);
            for j := 1 to MAX do
                sht[1].cells[row1, j + 1] := sum[1, j] / 100; //本章各套合计
            aq2.sql.clear;
            aq2.sql.add('select ' + field2 + ' from ' + TableName[1] +
                ' where [二级考点所属一级考点ID]=' + inttostr(i) +
                ' order by 二级考点ID ASC');
            ...
            if cou2 > 0 then
                ...
                begin
                    ...
                    getsum(2, str, Table);
                    for k := 1 to MAX do
                        Sht[2].cells[row2, k + 1] := sum[2, k] / 100;
                            //本二级小节各套合计
                    aq3.sql.clear;
                    aq3.sql.add('select ' + field3 + ' from ' + TableName[2] +
                        ' where ([三级考点所属一级考点ID]=' + inttostr(i) +
                        ...
                        begin
                            ...
                            if cou4 > 0 then
                                ...
                                begin
                                    ...
                                    sht[4].cells[row4, MAX + 2] := avg[4] / MAX;
                                        //平均值
            ...
            begin
                ...
                if sum[1, l] > 0 then
                    counter := counter + 1; //如果不是0就计数，准备计算概率
            end;
...
begin
    ...
    else {//如果当前不是C，也不是3级，那就是二级其他科目} if not is3ji then
        sqls := sqls + ' and ((XH>10 And XH<36) OR (XH>40))';
    ti.SQL.Add(sqls);
    ti.Open;
    ticou := ti.RecordCount;
    crow := 1;
...
begin
    if rg1.ItemIndex = 0 then
        ...
        table := '模拟题表';
    if MessageBox(0, PCHar('即将改写数据库中模拟题比例分析结果，' + #13 + #13 +
        '数据来源是“' + Table + '”，确定吗？'), '警告', MB_YESNO or
            MB_DEFBUTTON2 or
        MB_ICONWARNING) = mrno then
        Exit;
    if MessageBox(0, '确定一定以及肯定要改写数据库吗？', '警告将覆盖原有数据',
        MB_YESNO or MB_DEFBUTTON2 or MB_ICONWARNING) = mrno then
        Exit;
    if MessageBox(0, '操作将无法撤销！！！', '警告！！', MB_YESNO or
        MB_DEFBUTTON2
        or MB_ICONWARNING) = mrno then
        Exit;
  ...
  {    if is2c then begin
    ...
    ' 编号,一级考点名称,第一套模拟题分值比例,第二套模拟题分值比例,第三套模拟题分值比例,第四套模拟题分值比例,第五套模拟题分值比例,第六套模拟题分值比例,第七套模拟题分值比例,第八套模拟题分值比例,第九套模拟题分值比例 ';
    field2 :=
        ' 编号,二级考点名称,第一套模拟题分值比例,第二套模拟题分值比例,第三套模拟题分值比例,第四套模拟题分值比例,第五套模拟题分值比例,第六套模拟题分值比例,第七套模拟题分值比例,第八套模拟题分值比例,第九套模拟题分值比例 ';
    field3 :=
        ' 编号,三级考点名称,第一套模拟题分值比例,第二套模拟题分值比例,第三套模拟题分值比例,第四套模拟题分值比例,第五套模拟题分值比例,第六套模拟题分值比例,第七套模拟题分值比例,第八套模拟题分值比例,第九套模拟题分值比例 ';
    field4 :=
        ' 编号,四级考点名称,第一套模拟题分值比例,第二套模拟题分值比例,第三套模拟题分值比例,第四套模拟题分值比例,第五套模拟题分值比例,第六套模拟题分值比例,第七套模拟题分值比例,第八套模拟题分值比例,第九套模拟题分值比例 ';
    ...
    begin
        ...
        begin
            ...
            aq2.sql.add('select * from ' + TableName[1] +
                ' where [二级考点所属一级考点ID]=' + inttostr(i) +
                ' order by 二级考点ID ASC');
            aq2.open;
            cou2 := aq2.RecordCount;
            ...
            if cou2 > 0 then
                ...
                begin
                    ...
                    aq3.sql.add('select * from ' + TableName[2] +
                        ' where ([三级考点所属一级考点ID]=' + inttostr(i) +
                        ') and ([三级考点所属二级考点ID]=' + inttostr(j) +
                        ') order by 三级考点ID ASC');
                    aq3.open;
                    ...
                    if cou3 > 0 then
                        ...
                        begin
                            ...
                            aq4.sql.clear;
                            aq4.sql.add('select * from ' + TableName[3] +
                                ' where ([四级考点所属一级考点ID]=' + inttostr(i)
                                    +
                                ') and ([四级考点所属二级考点ID]=' + inttostr(j)
                                    +
                    ...
                    begin
                        sum[2, l] := sum[2, l] + sumer[3, l];
                        aq2['第' + numtohz(l) + '套模拟题分值比例'] := sum[2, l]
                            / 100;
    ...
    try
        ...
        begin
            v.range['b2:' + TableXiaBiao + '2'].FormatConditions.Add(1, 1, '0',
                '99.995');
            v.range['b2:' + TableXiaBiao +
                '2'].FormatConditions[2].Font.ColorIndex :=
                3;
            v.range['b2:' + TableXiaBiao + '2'].FormatConditions.Add(1, 7,
                '100.005');
        end
        ...
        v.range['b2:' + TableXiaBiao + '2'].FormatConditions[3].Font.ColorIndex
            :=
            3;
        v.range['b:' + TableXiaBiao].columnwidth := 5;
        sheet.range['b3'].select;
        ...
        begin
            ...
            if cou2 > 0 then
                ...
                begin
                    ...
                    getsum(2, str, Table);
                    for k := 1 to MAX do
                        Sheet.cells[crow, k + 1] := sum[2, k] / 100;
                    aq3.sql.clear;
                    aq3.sql.add('select ' + field3 + ' from ' + TableName[2] +
                        ' where ([三级考点所属一级考点ID]=' + inttostr(i) +
                        ') and ([三级考点所属二级考点ID]=' + inttostr(j) +
...
var
    ...
    field1, field2, field3, field4: string;
begin
    if MessageBox(0, '即将改写数据库中真题比例分析结果，确定吗？', '警告',
        MB_YESNO
        or MB_DEFBUTTON2 or MB_ICONWARNING) = mrno then
        Exit;
    if MessageBox(0, '确定一定以及肯定要改写数据库吗？', '警告将覆盖原有数据',
        MB_YESNO or MB_DEFBUTTON2 or MB_ICONWARNING) = mrno then
        Exit;
  ...
  //      field4:='*';
    ...
    begin
        ...
        MuX := 8;
        field1 :=
            ' 编号,一级考点名称,[2008年4月分值比例],[2008年9月分值比例],[2009年3月分值比例],[2009年9月分值比例],[2010年3月分值比例],[2010年9月分值比例],[2011年3月分值比例],[2011年9月分值比例] ';
        field2 :=
            ' 编号,二级考点名称,[2008年4月分值比例],[2008年9月分值比例],[2009年3月分值比例],[2009年9月分值比例],[2010年3月分值比例],[2010年9月分值比例],[2011年3月分值比例],[2011年9月分值比例] ';
        field3 :=
            ' 编号,三级考点名称,[2008年4月分值比例],[2008年9月分值比例],[2009年3月分值比例],[2009年9月分值比例],[2010年3月分值比例],[2010年9月分值比例],[2011年3月分值比例],[2011年9月分值比例] ';
        field4 :=
            ' 编号,四级考点名称,[2008年4月分值比例],[2008年9月分值比例],[2009年3月分值比例],[2009年9月分值比例],[2010年3月分值比例],[2010年9月分值比例],[2011年3月分值比例],[2011年9月分值比例] ';
    end
    else
    begin
        zt[1] := '2007年4月分值比例';
        ...
        MuX := 10;
        field1 :=
            ' 编号,一级考点名称,[2007年4月分值比例],[2007年9月分值比例],[2008年4月分值比例],[2008年9月分值比例],[2009年3月分值比例],[2009年9月分值比例],[2010年3月分值比例],[2010年9月分值比例],[2011年3月分值比例],[2011年9月分值比例] ';
        field2 :=
            ' 编号,二级考点名称,[2007年4月分值比例],[2007年9月分值比例],[2008年4月分值比例],[2008年9月分值比例],[2009年3月分值比例],[2009年9月分值比例],[2010年3月分值比例],[2010年9月分值比例],[2011年3月分值比例],[2011年9月分值比例] ';
        field3 :=
            ' 编号,三级考点名称,[2007年4月分值比例],[2007年9月分值比例],[2008年4月分值比例],[2008年9月分值比例],[2009年3月分值比例],[2009年9月分值比例],[2010年3月分值比例],[2010年9月分值比例],[2011年3月分值比例],[2011年9月分值比例] ';
        field4 :=
            ' 编号,四级考点名称,[2007年4月分值比例],[2007年9月分值比例],[2008年4月分值比例],[2008年9月分值比例],[2009年3月分值比例],[2009年9月分值比例],[2010年3月分值比例],[2010年9月分值比例],[2011年3月分值比例],[2011年9月分值比例] ';
    end;
    aq1 := TADOQuery.Create(nil);
    aq1.Connection := DBe.DM.con1;
    aq2 := TADOQuery.Create(nil);
...
begin
    if is3ji then
    begin
        MessageBox(0, '三级没有公共基础，谢谢!', '提示', MB_OK or
            MB_ICONWARNING);
        exit;
    end;
    ...
    begin
        MessageBox(0, PCHar('数据表:''' + PubTable[1] + '''似乎不存在!' + #13 +
            #13 +
            '请检查数据库!'),
            '出错了', MB_OK or MB_ICONWARNING);
        halt;
    ...
    try
        ...
        v.range['b2:' + TableXiaBiao + '2'].FormatConditions.Add(1, 1, '0',
            '9.995');
        v.range['b2:' + TableXiaBiao + '2'].FormatConditions[1].Font.ColorIndex
            :=
            3;
        v.range['b2:' + TableXiaBiao + '2'].FormatConditions.Add(1, 7,
            '10.005');
    ...
    aq4.connection := DBE.dm.con1;
    AQ1.sql.Clear;
    AQ1.SQL.Add('select ' + field1 + ' from ' + PubTable[0] +
        '  order by 一级考点ID ASC');
    AQ1.open;
    cou1 := AQ1.recordcount;
    avg[1] := 0;
    ...
    begin
        ...
        begin
            ...
            row1 := crow;
            v.range['a' + inttostr(crow) + ':' + TableXiaBiao +
                inttostr(crow)].interior.colorindex := 24; //颜色范围到K列
            str := aq1.fieldbyname('一级考点名称').asstring;
            Sheet.cells[crow, 1] := ' ' +
                aq1.fieldbyname('一级考点名称').asstring;
            str := copy(str, Pos(' ', str) + 1, Length(str) - pos(' ', str));
            ...
            for j := 1 to MAX do
                Sheet.cells[crow, j + 1] := sum[1, j] / 100;
            aq2.sql.clear;
            aq2.sql.add('select ' + field2 + ' from ' + PubTable[1] +
                ' where [二级考点所属一级考点ID]=' + inttostr(i) +
                ' order by 二级考点ID ASC');
            aq2.open;
            cou2 := aq2.RecordCount;
            ...
            if cou2 > 0 then
                ...
                begin
                    ...
                    for k := 1 to MAX do
                        Sheet.cells[crow, k + 1] := sum[2, k] / 100;
                    aq3.sql.clear;
                    aq3.sql.add('select ' + field3 + ' from ' + PubTable[2] +
                        ' where ([三级考点所属一级考点ID]=' + inttostr(i) +
                        ') and ([三级考点所属二级考点ID]=' + inttostr(j) +
                        ') order by 三级考点ID ASC');
                    aq3.open;
...
begin
    ...
    ti.SQL.Clear;
    ti.SQL.Add('select TH,XH,TM,DA,JX,考点1,OptionA,OptionB,OptionC,OptionD,编号 from '
        + tabl + ' where 考点1="' + kaod + '" '+mcondition+' order by TH,XH ASC');
    ti.open;
    cou := ti.RecordCount;
    with Form1 do
    begin
        ...
        begin
            ...
            edt1.Lines.Append('（第' + ti.fieldbyname('TH').AsString + '套第' +
                ti.fieldbyname('XH').AsString + '题）' +
                    ti.fieldbyname('编号').AsString);
            edt1.Lines.Append(ti.fieldbyname('TM').AsString);
            edt1.Lines.Append('A) ');
            edt1.Lines.Append(ti.fieldbyname('OptionA').AsString);
            edt1.Lines.Append('B) ');
            edt1.Lines.Append(ti.fieldbyname('OptionB').AsString);
            edt1.Lines.Append('C) ');
            edt1.Lines.Append(ti.fieldbyname('OptionC').AsString);
            edt1.Lines.Append('D) ');
            edt1.Lines.Append(ti.fieldbyname('OptionD').AsString);
            edt1.SelAttributes.Color := clGreen;
            if chk_disp_answer.checked then
                edt1.Lines.Append('答案：' + ti.fieldbyname('DA').AsString);
            if chk_disp_anay.Checked then
                edt1.Lines.Append(ti.fieldbyname('JX').AsString);
            if chk_sgn_answer.Checked then
               begin
                edtAnswer1.Lines.Append('（第' + ti.fieldbyname('TH').AsString + '套第' +
                ti.fieldbyname('XH').AsString + '题）' +
                    ti.fieldbyname('编号').AsString);
                edtAnswer1.Lines.Append('答案：' + ti.fieldbyname('DA').AsString);
                edtAnswer1.Lines.Append(ti.fieldbyname('JX').AsString);
               end;
...
begin
    ...
    begin
        ...
        begin
            ...
            if chk_sgn_answer.Checked then
                begin
                 edtAnswer1.SelAttributes.Size:=22;
                 edtAnswer1.Lines.Append(aq1.fieldbyname('一级考点名称').asstring);
               end;
            str := copy(str, Pos(' ', str) + 1, Length(str) - pos(' ', str));
            getTI(str, Table);
            aq2.sql.clear;
            ...
            if cou2 > 0 then
                ...
                begin
                   ...
                   if chk_sgn_answer.Checked then
                   begin
                    edtAnswer1.SelAttributes.Size := 18;
                    edtAnswer1.Lines.Append(aq2.fieldbyname('二级考点名称').asstring);
                   end;
                    str := aq2.fieldbyname('二级考点名称').asstring;
                    str := copy(str, Pos(' ', str) + 1, Length(str) - pos(' ',
                        str));
        ...
        aq2.Free;
        aq1.Free;
        edt1.Lines.SaveToFile(copy(TableName[0], 1, Length(TableName[0]) - 10) +
            '考点相关题.rtf');
        if chk_sgn_answer.Checked then
          edtAnswer1.Lines.savetofile(copy(TableName[0], 1, Length(TableName[0]) - 10) +
            '考点相关题答案及解析.rtf');
    end;
end;
...
begin
    if is3ji then
    begin
        MessageBox(0, '三级没有公共基础', '提示', MB_OK or MB_ICONINFORMATION);
        exit;
    end;
    PubTable[0] := '公共基础知识一级考点表';
    PubTable[1] := '公共基础知识二级考点表';
    ...
    begin
        ...
        aq2.Free;
        aq1.Free;
        edt1.Lines.SaveToFile(copy(PubTable[0], 1, Length(PubTable[0]) - 10) +
            '考点相关题.rtf');
        if chk_sgn_answer.Checked then
          edtAnswer1.Lines.savetofile(copy(PubTable[0], 1, Length(PubTable[0]) - 10) +
            '考点相关题答案及解析.rtf');
    end;
    caption := '完成';
...
begin
    ...
    else if TableExist(DBe.DM.con1, '模拟题表') then
