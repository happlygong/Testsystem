...
begin
  ...
  if FileExists('OperMDB.mdb') then
  begin
{   messagebox(0,'当前目录下没有需要的数据库文件<destiny.mdb>，'+#13+#13+
   ...
   str0:=str1+str2+'OperMDB.mdb'+str3;
   flag:=1;
  end else
  if FileExists('data\OperMDB.mdb') then
  begin
    //DM.AC1.ConnectionString:=str1+str2+'data\destiny.mdb'+str3;
    str0:=str1+str2+'data\OperMDB.mdb'+str3;
    flag:=2;
  end;
  //else
  if flag=0 then
